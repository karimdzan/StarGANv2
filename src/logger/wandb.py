...
class WanDBWriter:
    ...
    def add_model(self, model_path, artifact_name):
    ...
    def add_table_images(
        ...
        for orig, ref, out, rec in zip(
            original_images, reference_images, output_images, reconstructed_images
        ):
            image_table.add_data(
                wandb.Image(orig), wandb.Image(ref), wandb.Image(out), wandb.Image(rec)
            )
